...
<script type="text/javascript">
	$(function(){
		$.post("<?=site_url("ngadimin/produk?load=true")?>&page=1",{"cari":$("#cari").val(),[$("#names").val()]:$("#tokens").val()},function(msg){
			var data = eval("("+msg+")");
		...
		$("#cari").change(function(){
			...
			$.post("<?=site_url("ngadimin/produk")?>?page=1&load=true",{"cari":$("#cari").val(),[$("#names").val()]:$("#tokens").val()},function(msg){
				var data = eval("("+msg+")");
				updateToken(data.token);
				$(".card-body").html(data.result);
			});
	...
	function refreshTabel(page){
		$(".card-body").html('<i class="fas fa-spin fa-spinner"></i> Loading data...');
		$.post("<?=site_url("ngadimin/produk?load=true")?>&page="+page,{"cari":$("#cari").val(),[$("#names").val()]:$("#tokens").val()},function(msg){
			var data = eval("("+msg+")");
			updateToken(data.token);
			$(".card-body").html(data.result);
		});
