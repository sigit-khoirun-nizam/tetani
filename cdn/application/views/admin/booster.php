		...
		<div class="card-header row align-items-center">
			...
			<div class="col-md-4">
				<div class="btn-group col-12" role="group">
					<?php 
						$set = $this->func->globalset("notif_booster"); 
...
<script type="text/javascript">
		...
		$("#sbform").on("submit",function(e){
			...
			swal.fire({
				...
				if(vals.value){
					...
					$.post("<?=site_url("api/tambahbooster")?>",datar,function(msg){
						var data = eval("("+msg+")");
	...
	function saveSetting(val){
		...
		$.post("<?=site_url('api/savesetting')?>",{"notif_booster":val,[$("#names").val()]:$("#tokens").val()},function(ev){
			var data = eval("("+ev+")");
			updateToken(data.token);
			if(val == 1){
	...
	function loadSB(page){
		...
		$.post("<?=site_url("api/booster?load=true&page=")?>"+page,{"cari":$("#cari").val(),[$("#names").val()]:$("#tokens").val()},function(msg){
			var data = eval("("+msg+")");
			updateToken(data.token);
	...
	function edit(id){
		$.post("<?=site_url('api/booster')?>",{"formid":id,[$("#names").val()]:$("#tokens").val()},function(ev){
			var data = eval("("+ev+")");
			updateToken(data.token);
			$("#id").val(id);
	...
	function hapus(id){
		swal.fire({
			...
			if(vals.value){
				$.post("<?=site_url("api/hapusbooster")?>",{"id":id,[$("#names").val()]:$("#tokens").val()},function(msg){
					var data = eval("("+msg+")");
					updateToken(data.token);
					if(data.success == true){
						loadSB(1);
