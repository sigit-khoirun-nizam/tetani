...
<script type="text/javascript">
	...
	function loadPreorder(page){
		...
		$.post("<?=site_url("ngadimin/preorder?load=preorder&page=")?>"+page,{"cari":$("#cari").val(),[$("#names").val()]:$("#tokens").val()},function(msg){
			var data = eval("("+msg+")");
	...
	function loadProduk(page){
		...
		$.post("<?=site_url("ngadimin/preorder?load=produk&page=")?>"+page,{"cari":$("#cari").val(),[$("#names").val()]:$("#tokens").val()},function(msg){
			var data = eval("("+msg+")");
			updateToken(data.token);
			$("#load").html(data.result);
		});
	...
	function detailPesanan(id){
		$.post("<?=site_url("ngadimin/preorder?load=preorder&page=1&idproduk=")?>"+id,{[$("#names").val()]:$("#tokens").val()},function(msg){
			var data = eval("("+msg+")");
			updateToken(data.token);
