...
<div class="p-tb-10">
	<form id="kurirform">
		<div class="row m-lr-0">
			<?php
				$kurir = explode("|",$this->func->globalset("kurir"));
		...
		$(".kurir").each(function(){
			$(this).click(function(){
				...
				if($(this).data("aktif") == "no"){
					$.post("<?php echo site_url("api/aktifkankurir"); ?>",{"push":$(this).data("push"),[$("#names").val()]:$("#tokens").val()},function(msg){
						var data = eval("("+msg+")");
						updateToken(data.token);
						if(data.success == true){
					...
					if(hitungJumlah() > 1){
						$.post("<?php echo site_url("api/nonaktifkankurir"); ?>",{"push":$(this).data("push"),[$("#names").val()]:$("#tokens").val()},function(msg){
							var data = eval("("+msg+")");
							updateToken(data.token);
							if(data.success == true){
