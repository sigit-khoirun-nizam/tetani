...
<script type="text/javascript">
		...
		$("#voucherform").on("submit",function(e){
			...
			swal.fire({
				...
				if(vals.value){
					...
					$.post("<?=site_url("api/tambahvoucher")?>",datar,function(msg){
						var data = eval("("+msg+")");
	...
	function loadVoucher(page){
		...
		$.post("<?=site_url("api/voucher?load=true&page=")?>"+page,{"cari":$("#cari").val(),[$("#names").val()]:$("#tokens").val()},function(msg){
			var data = eval("("+msg+")");
			updateToken(data.token);
	...
	function edit(id){
		$.post("<?=site_url('api/voucher')?>",{"formid":id,[$("#names").val()]:$("#tokens").val()},function(ev){
			var data = eval("("+ev+")");
			updateToken(data.token);
			$("#voucherid").val(id);
			$("#nama").val(data.nama);
	...
	function hapus(id){
		swal.fire({
			...
			if(vals.value){
				$.post("<?=site_url("api/hapusvoucher")?>",{"id":id,[$("#names").val()]:$("#tokens").val()},function(msg){
					var data = eval("("+msg+")");
					updateToken(data.token);
					if(data.success == true){
						loadVoucher(1);
						...
						<select id="produk" name="idproduk" class="form-control" required >
							...
							<?php
								...
								foreach($dbs->result() as $rs){
									echo "<option value='".$rs->id."'>".$rs->nama." - Rp.".$this->func->formUang($rs->harga)."</option>";
								}
							?>
						</select>
					</div>
