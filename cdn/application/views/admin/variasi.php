...
<script type="text/javascript">
	$(function(){
		$("#warna").load("<?=site_url("ngadimin/variasi?load=warna")?>");
		$("#size").load("<?=site_url("ngadimin/variasi?load=size")?>");
		...
		$("#simpan").on("submit",function(e){
			...
			$.post("<?=site_url("ngadimin/tambahvariasi")?>",datar,function(e){
				var data = eval("("+e+")");
	...
	function refreshWarna(page){
		$("#warna").load("<?=site_url("ngadimin/variasi?load=warna&page=")?>"+page);
	}
	function refreshSize(page){
		$("#size").load("<?=site_url("ngadimin/variasi?load=size&page=")?>"+page);
	}
	...
	function hapusSize(id){
		swal.fire({
			...
			if(val.value == true){
				$.post("<?=site_url("ngadimin/hapussize")?>",{"theid":id,[$("#names").val()]:$("#tokens").val()},function(msg){
					var data = eval("("+msg+")");
					updateToken(data.token);
					if(data.success == true){
						swal.fire("Berhasil","data telah dihapus","success").then((val)=>{
							window.location.href="<?=site_url("ngadimin/variasi")?>";
						});
					}else{
	...
	function hapusWarna(id){
		swal.fire({
			...
			if(val.value == true){
				$.post("<?=site_url("ngadimin/hapuswarna")?>",{"theid":id,[$("#names").val()]:$("#tokens").val()},function(msg){
					var data = eval("("+msg+")");
					updateToken(data.token);
					if(data.success == true){
						swal.fire("Berhasil","data telah dihapus","success").then((val)=>{
