...
<script type="text/javascript">
		...
		$("#rekeningform").on("submit",function(e){
			...
			swal.fire({
				...
				if(vals.value){
					...
					$.post("<?=site_url("api/updateblog")?>",datar,function(msg){
						var data = eval("("+msg+")");
	...
	function loadBlog(page){
		...
		$.post("<?=site_url("ngadimin/blog?load=hal&page=")?>"+page,{"cari":$("#cari").val(),[$("#names").val()]:$("#tokens").val()},function(msg){
			var data = eval("("+msg+")");
			updateToken(data.token);
	...
	function hapus(pro){
		swal.fire({
			...
			if(result.value){
				$.post("<?php echo site_url('api/hapusblog'); ?>",{"id":pro,[$("#names").val()]:$("#tokens").val()},function(msg){
					var data = eval("("+msg+")");
					updateToken(data.token);
					if(data.success == true){
