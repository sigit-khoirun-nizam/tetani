	...
	<?php
		...
		if($rows > 0){
			...
			foreach($db->result() as $r){
				...
				if($r->bukti != ""){
					$status .= "<br/><a href='javascript:void(0)' onclick='bukti(\"".base_url("konfirmasi/".$r->bukti)."\")'>&raquo; Lihat Bukti Transfer</a>";
				}
	?>
			<tr>
				<td>#<?=$r->invoice."<br/>".$status?></td>
				<td><?=$this->func->ubahTgl("d/m/Y H:i",$r->tgl)?></td>
				<td><?=$this->func->getUserdata($r->usrid,"nama")?></td>
				<td><?=$this->func->getProduk($r->idproduk,"nama")?></td>
...
<div class="table-responsive">
	...
	?>
			<tr>
				<td><?=$no?></td>
				<td><?=$r->nama?></td>
				<td><?=$this->func->formUang($kuota)."<br/>[".$this->func->formUang($kuotatotal)."]"?></td>
				<td><?=$this->func->formUang($kuotab)."<br/>[".$this->func->formUang($kuotabtotal)."]"?></td>
				<td><?=$this->func->formUang($kuotas)."<br/>[".$this->func->formUang($kuotastotal)."]"?></td>
				<td>
