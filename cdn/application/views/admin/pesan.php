...
<script type="text/javascript">
	...
	function loadBaca(page){
		...
		$.post("<?=site_url("ngadimin/pesan?load=baca&page=")?>"+page,{"cari":$("#cari").val(),[$("#names").val()]:$("#tokens").val()},function(msg){
			var data = eval("("+msg+")");
	...
	function loadSemua(page){
		$("#load").html('<i class="fas fa-spin fa-spinner"></i> Loading data...');
		$.post("<?=site_url("ngadimin/pesan?load=semua&page=")?>"+page,{"cari":$("#cari").val(),[$("#names").val()]:$("#tokens").val()},function(msg){
			var data = eval("("+msg+")");
			updateToken(data.token);
			$("#load").html(data.result);
		});
...
<script type="text/javascript">
	$(function(){
		$("#kirimpesan").on("submit",function(e){
			...
			$.post("<?=site_url("api/kirimpesan")?>",datar,function(s){
				var data = eval("("+s+")");
				updateToken(data.token);
				$("#isipesan").val("");
