...
<script type="text/javascript">
	$(function(){
        //loadRiwayat();
        $("#usrid").load("<?=site_url("api/getusrid")?>?level=1");
        
        $("#level").change(function(){
            $("#usrid").load("<?=site_url("api/getusrid")?>?level="+$(this).val());
        });
	...
	function loadRiwayat(){
		...
		$.post("<?=site_url("ngadimin/laporanuser?load=hal")?>",{"usrid":$("#usrid").val(),"tglmulai":$("#tglmulai").val(),"tglselesai":$("#tglselesai").val(),[$("#names").val()]:$("#tokens").val()},function(msg){
			var data = eval("("+msg+")");
