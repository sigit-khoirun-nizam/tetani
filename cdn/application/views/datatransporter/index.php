<div class="container">
    ...
    <nav aria-label="breadcrumb">
        <ol class="breadcrumb ">
            ...
            <li class="breadcrumb-item active" aria-current="page">List Data Transporter</li>
        </ol>
    </nav>
    <div class="row">
        <div class="col-md-12">
            <a class="btn btn-primary mb-2" href="<?= base_url('datatransporter/add'); ?>">Tambah Data</a>
            <div mb-5>
                        ...
                        <table class="table table-striped table-bordered table-hover" id="tableData">
                            ...
                            <tbody>
                                <?php foreach ($data_transporter as $row) : ?>
                                    <tr>
                                        <td><?= $row->kode_transporter ?></td>
                                        <td><?= $row->nama ?></td>
                                        <td><?= $row->no_hp ?></td>
                                        <td><?= $row->kota ?></td>
                                        <td><?= $row->status ?></td>
                                        <td>
                                            <a href="<?= site_url('datatransporter/edit/' . $row->id) ?>" class="btn btn-success btn-sm"><i class="fa fa-edit"></i> </a>
    ...
    $('#tableData').on('click', '.item-delete', function() {
        ...
        $('#btdelete').unbind().click(function() {
            $.ajax({
                ...
                method: 'get',
                async: false,
                url: '<?php echo base_url() ?>datatransporter/delete',
                data: {
                    id: id
                },
                dataType: 'json',
