...
class DataTransporter_model extends CI_Model
{
    ...
    {
        $this->db->select('RIGHT(blw_d_transporter.kode_transporter,2) as kode_transporter', FALSE);
        $this->db->order_by('kode_transporter', 'DESC');
        $this->db->limit(1);

        $query = $this->db->get('blw_d_transporter');
        if($query->num_rows() <> 0){      
            //cek kode jika telah tersedia    
            $data = $query->row();      
            $kode = intval($data->kode_transporter) + 1; 
       }
       else{      
